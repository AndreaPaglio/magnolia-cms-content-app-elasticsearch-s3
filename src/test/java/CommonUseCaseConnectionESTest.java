...
class CommonUseCaseConnectionESTest {
	...
	void add() throws GenericException {
		Params addParams = GenericParamsBuilder.createAdd(Log.class)
				.addField("id", String.valueOf(Math.random() * 1000)).addField("level", "13").get();

		AddItem<Log> addItem;
		try {
			addItem = new AddItem<Log>(
					FieldUtils.createInstanceFromClassAndValues(logClass, addParams, logClass.newInstance()), logClass);
		} catch (InstantiationException | IllegalAccessException e) {
			throw new GenericException("Error to Add");
		}
	...
	void search() throws GenericException {
		...
		searchParams = GenericParamsBuilder.createSearch().size(100).get();
		results = null;
